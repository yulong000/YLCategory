...
{
    return [[NSDate today] indexOfWeekInYear];
}
...
{
    NSDate *startDate = nil;
    BOOL ok = [[NSCalendar currentCalendar] rangeOfUnit:NSCalendarUnitMonth startDate:&startDate interval:NULL forDate:[NSDate today]];
    NSAssert1(ok, @"Failed to calculate the first day of the month based on %@", self);
    return startDate;
}
...
{
    NSCalendarUnit calendarUnit = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay;
    NSDateComponents *dateComponents = [[NSCalendar currentCalendar] components:calendarUnit fromDate:[NSDate today]];
    dateComponents.day = [[NSCalendar currentCalendar] rangeOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitMonth forDate:[NSDate today]].length;;
    return [[NSCalendar currentCalendar] dateFromComponents:dateComponents];
}
...
{
    NSDateComponents *dateComponents = [[NSCalendar currentCalendar] components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond fromDate:self];
    return dateComponents;
}
#pragma mark 获取今天日期(当前的时区)
+ (NSDate *)today
{
    // 对时区引起的差别进行转换
    NSInteger interval = [[NSTimeZone localTimeZone] secondsFromGMT];
    NSDate *today = [[NSDate alloc] initWithTimeInterval:interval sinceDate:[NSDate date]];
    return today;
}
